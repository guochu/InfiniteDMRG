...
struct TransferMatrix{M <: MPSTensor}
	state::Vector{M}
end
...
function transfer_left(left::MPSBondTensor, m::TransferMatrix)
	for a in m.state
		left = updateleft(left, a, a)
	end
	return left
end

function transfer_right(right::MPSBondTensor, m::TransferMatrix)
	for a in Iterators.reverse(m.state)
		right = updateright(right, a, a)
	end
	return right
end
...
function random_boundaries(x::TransferMatrix) 
	T = scalartype(x)
	vl = TensorMap(randn, T, space_l(x.state[1]), space_l(x.state[1]))
	vr = TensorMap(randn, T, space_r(x.state[end])', space_r(x.state[end])')
	return vl * vl', vr * vr'
end
